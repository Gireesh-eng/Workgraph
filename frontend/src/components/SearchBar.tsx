  ...
  return <div className="relative w-full" onFocus={() => setShowSuggestions(true)} onBlur={() => setTimeout(() => setShowSuggestions(false), 200)}>
    <form onSubmit={e => { e.preventDefault(); setShowSuggestions(false); if (query.trim()) navigate(`/search?q=${encodeURIComponent(query.trim())}`); }} className="w-full">
      <div className={`flex h-11 items-center rounded-control border shadow-inner focus-within:ring-[3px] ${dark ? "border-white/[.15] bg-white/[.1] focus-within:border-white/40 focus-within:ring-white/10" : "border-border bg-surface-sunken shadow-[#6c5140]/[.03] focus-within:border-ink-900 focus-within:ring-ink-900/10"}`}>
        ...
        <input ref={ref} value={query} onChange={e => setQuery(e.target.value)} placeholder="Search people, projects, tasks…" autoComplete="off" autoCorrect="off" autoCapitalize="off" spellCheck="false" className={`h-full min-w-0 flex-1 bg-transparent px-3 text-[14px] font-medium outline-none ${dark ? "text-white placeholder:text-white/40" : "text-ink-900 placeholder:font-normal placeholder:text-ink-400"}`} />
    ...
    {showSuggestions && query.trim().length <= 1 && (
      <div className="absolute left-0 right-0 top-[calc(100%+6px)] z-50 overflow-hidden rounded-xl border border-border bg-paper shadow-xl motion-safe:animate-fade-up" style={{ animationDuration: "150ms" }}>
        <div className="px-4 py-3 text-[11px] font-semibold text-ink-400 uppercase tracking-wider">Suggested Searches</div>
        <ul className="py-2 pt-0">
          {SUGGESTED_SEARCHES.map((term) => (
            <li key={term}>
              <button type="button" onMouseDown={(e) => { e.preventDefault(); setQuery(term); navigate(`/search?q=${encodeURIComponent(term)}`); setShowSuggestions(false); }} className="w-full flex items-center gap-3 px-4 py-2.5 text-left text-[14px] font-medium text-ink-900 transition-colors hover:bg-surface-hover">
                <svg className="h-4 w-4 shrink-0 text-ink-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="1.5"><circle cx="11" cy="11" r="6" /><path d="m16 16 4 4" /></svg>
                {term}
              </button>
            </li>
          ))}
